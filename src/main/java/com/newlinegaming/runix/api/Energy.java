...
import com.newlinegaming.runix.api.internal.BlockDescription;
...
public class Energy {
    ...
    public Energy() {
        /**
         * naturalBlocks is an important list because it lists all blocks that
         * will not conduct runic energy
         */
...
//                add(ModBlock.greekFire);
        ...
        crushableBlocks = new ArrayList<Block>() {
            {// torches are debatable, since someone did place it there
                add(Blocks.air);
                add(Blocks.deadbush);
                add(Blocks.snow_layer);
                add(Blocks.fire);
...
//                add(ModBlock.greekFire);
    ...
    public void initializeEnergyRegistry() {
        ...
        addBlock(Blocks.acacia_door, 48); // TODO: Add more door
        ...
        addBlock(Blocks.dark_oak_fence, 12); // TODO: add more fences
    ...
    public static void addBlock(Block block, int energy) {
        addBlock(block, energy, naturalBlocks.contains(block), crushableBlocks.contains(block),
                moveSensitiveBlocks.contains(block)); // call more detailed
                                                      // method
    }
    ...
    public static int energyToRadiusConversion(int energy) {
        return Energy.energyToRadiusConversion(energy, blockMoveCost);
    }

    public static int energyToRadiusConversion(int energy, float perBlockCost) {
        int diameter = 1;
        while (diameter * diameter * diameter * perBlockCost < energy) // this
                                                                       // is
                                                                       // over
                                                                       // generous
                                                                       // intentionally
            diameter += 2; // +2 so that we always have an odd number and have
                           // block centered shapes
        return diameter / 2; // integer math will round down the .5
    }

    /**
     * naturalBlocks is an important list because it lists all blocks that will
     * not conduct runic energy
     */
    ...
    /**
     * This is a list of all the blocks that need special treatment when moving
     * groups of blocks like FTP or Runecraft. All independent blocks need to be
     * placed first because all of these blocks attach to other blocks or (in
     * the case of liquids) need to be held in by solid blocks.
     * 
     * @param blockID
     */
