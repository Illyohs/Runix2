...
import com.newlinegaming.runix.api.internal.block.FuelBlock;
import com.newlinegaming.runix.api.internal.block.NoneBlock;
import com.newlinegaming.runix.api.internal.block.SignatureBlock;
import com.newlinegaming.runix.api.internal.block.TierBlock;
...
import com.newlinegaming.runix.common.rune.persistant.WaypointRune;
import com.newlinegaming.runix.common.rune.timed.BuildMasterRune;
	            ...
	            {//begin scanning downward
	                ...
	                { //We found something not AIR
    	                if (block == Blocks.lava || block == Blocks.flowing_lava//check for Lava, fire, and void
    	                    ...
    	                    spendEnergy((int)( coords.getDistance(dCalc) * Energy.movementPerMeterCost));
...
//            System.out.println(patternID.blockID + " should be " + blockID);
    ...
    protected int getTier(WorldPos coords){
        ...
        return blockID != null ? Energy.getTier(blockID) : 1;
    }
          ...
          //tear detection: this should be empty by the last step
            ...
            for(WorldPos block : activeEdge) {
                ...
                for(WorldPos n : neighbors) {
                    ...
                    if( !workingSet.contains(n) && !Energy.isNatural(blockID) ) {
                        workingSet.add(n);
                        nextEdge.add(n);
                    }
                }
...
//                        shape.put(target, target.getState());
    ...
    protected void consumeRune(WorldPos coords) {
        ...
        for( WorldPos target : shape.keySet()){
            ...
            Block blockID = target.getBlock();
            energy += Energy.getEnergy(blockID);//convert ID into energy
    ...
    public void setBlockIdAndUpdate(WorldPos coords, Block sourceBlock) throws NotEnoughRunicEnergyException {
        if( sourceBlock == Blocks.air )//this is actually breaking, not paying for air
            spendEnergy(Energy.blockBreakCost);
        else
            spendEnergy(Energy.getEnergy(sourceBlock.getBlockState().getBlock()));
    ...
    public void setBlockIdAndUpdate(WorldPos destination, IBlockState sourceBlock) throws NotEnoughRunicEnergyException {
        if( sourceBlock.getBlock() == Blocks.air )//this is actually breaking, not paying for air
            ...
            spendEnergy(Energy.getEnergy(sourceBlock.getBlock()));
    ...
    public void moveBlock(WorldPos coords, WorldPos newPos) throws NotEnoughRunicEnergyException {
        ...
        spendEnergy((int) Energy.blockMoveCost);
    ...
    protected boolean consumeFuelBlock(WorldPos coords) {
        if(Energy.getTier(coords.getBlock()) > 1){
            energy += Energy.getEnergy(coords.getBlock());
    ...
    protected String shortClassName() {
        return this.getClass().toString().replace("class com.newlinegaming.runix.common.rune.", "");
    }
