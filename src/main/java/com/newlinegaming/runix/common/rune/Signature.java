...
public class Signature {
     ...
     */
    public Signature(AbstractRune rune, WorldPos coords) {
        blocks = new ArrayList<IBlockState>();
        //this list specifically lacks any block that uses meta for orientation
        Block[] metaWhiteList = new Block[]{
                Blocks.carpet, 
                Blocks.wool,
                Blocks.wheat, //added just in case you WANT an impossible waypoint
                Blocks.hardened_clay, 
                Blocks.jukebox, //adjusting the notes would change your Signature :D
                Blocks.planks,
                Blocks.heavy_weighted_pressure_plate,
                Blocks.light_weighted_pressure_plate, 
                Blocks.wooden_pressure_plate, 
                Blocks.stone_pressure_plate, //may be some potential there
                Blocks.stained_hardened_clay,
                Blocks.log, Blocks.wooden_slab, 
                Blocks.double_wooden_slab,
                Blocks.stained_glass, 
                Blocks.stained_glass_pane
        };

        HashMap<WorldPos, IBlockState> shape = rune.runicFormulae(coords);
...
//                        blocks.add(target.getState());
                        blocks.add(target.getState());
                    } else {                        
//                        blocks.add(new SigBlock(blockID, 0));//just the blockID
                        // No more Id's only meta
//                        blocks.add(target.stat);
                    }
                }
            }
        }
