...
public class FaithRune extends PersistentRune{
    ...
    protected void poke(EntityPlayer poker, WorldPos coords) {
        if(firstTime){// firstTime prevents players from injecting more energy by building a second rune on top of the first
            ...
            BlockUtils.replaceBlock(coords.getWorld(), coords, coords.getBlock(), ModBlock.faithAnchor);
            energy -= Energy.getEnergy(Blocks.gold_block) * 5; //the Gold blocks don't count towards the energy
            radius = Energy.energyToRadiusConversion(energy);
            HashSet<WorldPos> tsphere = fullStructure();
            energy -= tsphere.size() * Energy.blockMoveCost;
