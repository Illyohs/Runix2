...
public abstract class PersistentRune extends BaseRune
{
    ...
    public PersistentRune(WorldPos coords, EntityPlayer activator, String runeName)
    {
        location = coords;
        setPlayer(activator);
...
//			Class
     ...
     * <p>
     * NOTE: It is important that you implement constructor YourRune(WorldPos coords, EntityPlayer activator)
     * even if it is only to call super(coords, activator) in order for persistence to work correctly.
     */
    @Override
    public void execute(WorldPos coords, EntityPlayer activator)
    {
        execute(coords, activator, Vector3.UP); //Up is the default
    }

    public void execute(WorldPos coords, EntityPlayer activator, Vector3 forward)
    {
    ...
    protected PersistentRune getOrCreateRune(WorldPos coords, EntityPlayer activator)
    {
        ...
        if (match == null)
        {//can't find anything: create a new one
            try
            {//this is a Java trick called reflection that grabs a constructor based on the parameters
                match = this.getClass().getConstructor(WorldPos.class, EntityPlayer.class).newInstance(coords, activator);
                getActiveMagic().add(match);//add our new Rune to the list
            } catch (Exception e)
            {
                System.err.println("This Persistent runes require a constructor YourRune(WorldPos loc, EntityPlayer user) to be defined.");
                e.printStackTrace();
                return null;
            }
        }
     ...
     * Return the rune in getActiveMagic() that matches the given coordinates or null if there is none
     */
    public PersistentRune getRuneByLocation(WorldPos coords)
    {
    ...
    public void moveMagic(HashMap<WorldPos, WorldPos> positionsMoved)
    {
        for (PersistentRune rune : getActiveMagic())
        {
            ...
            {//grab the destination keyed by source position
    ...
    protected HashSet<WorldPos> attachedStructureShape(EntityPlayer activator)
    {
        return attachedStructureShape(activator, fullStructure());
    }

    protected HashSet<WorldPos> attachedStructureShape(EntityPlayer activator, HashSet<WorldPos> scannedStructure)
    {
        if (activator != null)
        {
    ...
    public HashSet<WorldPos> directConductanceStructure()
    {
        int               tier             = getTier();
        HashSet<WorldPos> scannedStructure = conductanceStep(location, tier);
        return scannedStructure;
    }
    ...
    public void moveStructureAndPlayer(EntityPlayer player, WorldPos destination, HashSet<WorldPos> structure)
    {
        try
        {
            WorldPos destinationCenter = Util_Movement.safelyTeleportStructure(structure, location, destination, getTier());
